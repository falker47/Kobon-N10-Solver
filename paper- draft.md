# Computational Search for the N=10 Kobon Triangle Problem: A Stochastic Basin-Hopping Approach
...
## Abstract
The Kobon Triangle problem, proposed by Kobon Fujimura in 1978, asks for the maximum number of non-overlapping triangular regions $K(N)$ that can be formed by $N$ lines in general position in the Euclidean plane. The known upper bound is $K(N) \leq \lfloor N(N-2)/3 \rfloor$. For $N=10$, this yields a theoretical maximum of 26 triangles; however, the best construction to date achieves only 25 (T. Suzuki, 2002), leaving a gap of unknown origin.

This paper presents a systematic computational investigation of the $N=10$ case using **Stochastic Basin Hopping**—a hybrid approach combining Simulated Annealing for global exploration with iterative local refinement. Across 20 independent optimization runs, we achieved a **100% convergence rate** to the known record of 25 triangles. More significantly, geometric clustering analysis revealed **52 distinct families** of optimal configurations, demonstrating that the solution space is not singular but rather an archipelago of equivalent local optima.

Furthermore, topological sensitivity analysis ("Breather Scans") showed that the 25-triangle configurations occupy extremely narrow basins, collapsing to sub-optimal scores with perturbations as small as 2%. We conclude that the barrier from 25 to 26 is likely **topological rather than metric**, suggesting that future progress requires combinatorial graph-theoretic approaches rather than continuous optimization.
...
### 1.3 Contribution
This paper provides a **computational verification** of the $N=10$ landscape using modern stochastic optimization. Our contributions are:
...
$$\mathbf{S}' = \mathbf{S} + \mathcal{N}(0, \sigma \cdot T_k), \quad \sigma = 0.1$$

After perturbation, the lines are re-normalized to satisfy $a_i^2 + b_i^2 = 1$.
...
**Vectorized Intersection Matrix.** Rather than computing intersections pairwise in nested loops, we exploit broadcasting to compute the full $N \times N \times 2$ intersection tensor in a single vectorized operation:
...
A critical finding of this study is that the 25-triangle solution is **not unique**. Through systematic collection and clustering, we identified **52 geometrically distinct families** of optimal configurations.

**Clustering Methodology.** To determine geometric equivalence, we applied the following canonicalization pipeline:

1. **Normalization:** Ensure all lines satisfy $a^2 + b^2 = 1$ with consistent sign convention (first non-zero coefficient positive).
2. **Sorting:** Order lines lexicographically by $(a, b, c)$ to eliminate permutation ambiguity.
3. **Distance Metric:** Compute the Frobenius norm between sorted parameter matrices:
   $$d(\mathbf{S}_1, \mathbf{S}_2) = \left\| \text{sort}(\mathbf{S}_1) - \text{sort}(\mathbf{S}_2) \right\|_F$$
4. **Clustering Threshold:** Two configurations are considered equivalent if $d < 0.05$.

**Results Summary:**

| Category | Count |
|----------|-------|
| Total Configurations Collected | 72 |
| Unique Geometric Families | 52 |
| Largest Family (Most Common Attractor) | 9 members |
| Singleton Families (Unique Solutions) | 43 |

**Interpretation.** The existence of 52 distinct families contradicts the hypothesis of a unique global optimum. The solution space resembles an **archipelago**—a collection of isolated peaks at the same altitude (Score = 25), separated by valleys of lower-scoring configurations. This explains why different random initializations converge to structurally different solutions.

### 3.3 Topological Sensitivity Analysis ("The Breather Test")
...
We tested this hypothesis through two experiments:
...
2. **Archipelago Structure:** There is no unique solution; 52 distinct geometric families achieve the same score.
3. **Narrow Peaks:** Each family occupies an extremely narrow basin (FWHM < 2% in parameter space).
4. **No Gradient to 26:** Perturbation experiments show no "smooth path" from 25 to 26.

We conclude that **25 is the global maximum** for the connected component of the solution space reachable via continuous deformation from random initialization.

**Conjecture.** If a configuration achieving $K(10) = 26$ exists, it must reside in a **topologically isolated basin** characterized by:
- A fundamentally different intersection graph (combinatorial structure).
...
This paper presents a comprehensive computational investigation of the Kobon Triangle problem for $N=10$ lines. Our principal findings are:
...
2. **Rich Solution Landscape:** We discovered **52 geometrically distinct families** of optimal configurations, demonstrating that the solution space is an archipelago of equivalent local optima rather than a single unique peak.

3. **Topological Isolation:** Sensitivity analysis revealed that optimal configurations occupy extremely narrow basins in parameter space. Perturbations as small as 2% cause the score to collapse, indicating that there is no smooth gradient path to higher scores.
...
**Final Verdict.** We have computationally verified the robustness of the $K(10) = 25$ record using modern stochastic optimization. The barrier from 25 to 26 appears to be **topological rather than metric**: it is not a matter of precision or computational power, but of fundamentally different geometric structure. If a 26-triangle configuration for $N=10$ exists, it lies beyond the reach of continuous optimization and awaits discovery through combinatorial or algebraic methods.
